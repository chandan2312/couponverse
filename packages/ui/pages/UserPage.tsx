...
const UserPage = async ({
  ...
  const [userRes, couponsRes, offersRes] =
    ...
    await Promise.all([
      ...
      // coupons
      axios.get(
        `${process.env.NEXT_PUBLIC_BACK_URL}/coupon/getMany?username=${slug}&orderby=updatedAt_desc&country=${country}&lang=${lang}&page=1&perpage=${perpage}&morefields=viewsArr,upvotesArr,downvotesArr,store`,
      ),
      // offers
      axios.get(
        `${process.env.NEXT_PUBLIC_BACK_URL}/offer/getMany?username=${slug}&orderby=updatedAt_desc&country=${country}&lang=${lang}&page=1&perpage=${perpage}&morefields=upvotesArr,downvotesArr,store`,
      ),
    ]);
