...
import { englishCountries } from "../constants";
import { checkEnglishCountryArray, getLang } from "../lib/utils";
...
export const getStoreCount = cache(async (country) => {
...
export const getStoreList = cache(async (country, take, skip) => {
  try {
...
export const getStorePage = cache(async (slug, lang, country) => {
  try {
    ...
    console.log(store.coupons.length);
    ...
    if (!country) {
      const access = checkEnglishCountryArray(store.access);
      if (!access) {
        return {
          status: 404,
          message: "Store Not Found",
...
export const getTrendingStores = async (country) => {
  const sevenDaysAgo = new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  try {
...
export const getLatestStores = async (country) => {
  try {
    const result = await prisma.Store.findMany({
      where: {
        access: {
